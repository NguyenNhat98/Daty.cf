...
use App\Daty\Components\WpAdmin\User\Models\{ User, UserProfile, UserAuthorityModel, LikeDislikeModal, CreditWalletTransaction, UserSubscription, ProfileBoost, UserBlock };
use App\Daty\Components\WpAdmin\User\Models\NotificationLog;
    ...
    if (!function_exists('getStoreSettings')) {
        function getStoreSettings($itemName)
        {
            ...
            $storeConfiguration = Cache::rememberForever('cache.app.setting.all', function () use ( $appSettings) {
                ...
                $configurationSettings = \App\Daty\Components\WpUser\Configuration\Models\ConfigurationModel::select('name', 'value', 'data_type')->get();
    ...
    *---------------------------------------------------------------- */
    ...
    if (!function_exists('getUserSettings')) {
        function getUserSettings($itemName, $userID = null)
        {
            ...
            $userSettingConfiguration = \App\Daty\Components\WpUser\UserSetting\Models\UserSettingModel::select('key_name', 'value', 'data_type')->where('users__id', $userID)->get();
