...
use Carbon\Carbon;
    ...
    Validator::extend('unique_title', function($attribute, $value, $parameters)
    {
        ...
        $account = App\Daty\Services\ServiceControl\Models\AccountModel::where('title', $title)->first();
    ...
    Validator::extend('unique_email', function($attribute, $value, $parameters)
    {
        ...
        $user = App\Daty\Components\WpAdmin\User\Models\User::where('email', $email)->first();
        $accountUser = null;
        ...
        if (!__isEmpty($user)) {
            ...
            $accountUser = App\Daty\Components\WpAdmin\User\Models\UserAuthorityModel::where([
                'users__id' => $userId
            ])->first();
        }
    ...
    Validator::extend('ssh_public_key', function ($attribute, $value, $parameters, $validator) {
        return App\Daty\Support\Utils::validatePublicKey($value);
    });
    ...
    Validator::extend('verfy_authenticator_code', function( $attribute, $value, $parameters)
    {
        ...
        $twoFactorAuthKey = App\Daty\Components\WpAdmin\User\Models\UserModel::where('email', $email)->pluck('2fa_enabled')->toArray();
    ...
    Validator::extend('unique_page_title', function($attribute, $value, $parameters)
    {
        ...
        $title = Str::slug($value, '-');
		$page = App\Daty\Components\WpUser\Pages\Models\PageModel::where('title', $inputData['title'])->where('_uid', '!=', $inputData['pageUid'])->first();
		// check is empty
        if (!__isEmpty($page)) {
            return false;
        }
