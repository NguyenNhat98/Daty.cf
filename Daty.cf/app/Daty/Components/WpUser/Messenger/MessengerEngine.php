...
namespace App\Daty\Components\WpUser\Messenger;
...
use PushBroadcast;
use Carbon\Carbon;
use App\Daty\Components\WpUser\Messenger\Repositories\MessengerRepository;
...
use App\Daty\Components\WpUser\User\Repositories\CreditWalletRepository;
...
class MessengerEngine extends BaseEngine
{
    ...
    public function processSendMessage($inputData, $userId)
    {
        $transactionResponse = $this->messengerRepository->processTransaction(function() use ($inputData, $userId) {
            ...
            if (\__isEmpty($messageRequest)) {
                notificationLog('Yêu cầu tin nhắn từ '.' '.getUserAuthInfo('profile.full_name'), route('user.profile_view', ['username' => getUserAuthInfo('profile.username')]), null, $userDetails->_id);
                $initialMessageGeneratedUid = YesSecurity::generateUid();
                $isMessageRequestReceived = true;
                $initialMessageRequest = [
                    [
                        'status' => 1, // Sent
                        'message' => 'Yêu cầu tin nhắn',
                        'type' => 9,
            ...
            if ($addedMessagesUid = $this->messengerRepository->storeMessage($chatData)) {
                ...
                PushBroadcast::notifyViaPusher('event.user.chat.messages' ,[
                    ...
                    'notificationMessage'   => __tr('Nhận được tin nhắn mới từ __fullName__.', [
    ...
    public function processAcceptDeclineMessageRequest($inputData, $userId)
    {
        ...
        if (!\__isEmpty($pendingMessageRequestCollection)
            ...
            if ($this->messengerRepository->updateMessages($messageUpdateData)) {
                $message = 'Yêu cầu tin nhắn được chấp nhận, bây giờ bạn có thể trò chuyện với nhau';
                $type = 10;
                $messageRequestStatus = 'MESSAGE_REQUEST_ACCEPTED';
                ...
                PushBroadcast::notifyViaPusher('event.user.chat.messages' ,[
                    ...
                    'notificationMessage'   => ($type == 10)
                                                ? __tr('Yêu cầu tin nhắn được chấp nhận bởi __fullName__.', [
                                                    '__fullName__' => $loggedInUserFullName
                                                ])
                                                : __tr('Yêu cầu tin nhắn bị từ chối bởi __fullName__.', [
                                                    '__fullName__' => $loggedInUserFullName
                                                ])
                ]);
        ...
        return $this->engineReaction(2, null, 'Đã xảy ra sự cố trên máy chủ');
    }
    ...
    public function processBuySticker($inputData)
    {
        $transactionResponse = $this->messengerRepository->processTransaction(function() use($inputData) {
            ...
            if (\__isEmpty($stickerData)) {
                return $this->messengerRepository->transactionResponse(2, ['show_message' => true], 'Hình dán không tồn tại');
            }
            // Get User sticker
            ...
            if (!\__isEmpty($userItem)) {
                return $this->messengerRepository->transactionResponse(2, ['show_message' => true], 'Bạn đã mua hình dán này');
            }
            // fetch user credits
            ...
            if ($stickerData->normal_price > $totalUserCredits) {
                return $this->messengerRepository->transactionResponse(2, ['show_message' => true], 'Hạn mức tài khoản của bạn quá thấp, vui lòng nạp thêm tiền vào tài khoản');
            }
            ...
            if ($isStickerPurchase) {
                ...
                ], 'Đã mua hình dán thành công');
            }

            return $this->messengerRepository->transactionResponse(2, ['show_message' => true], 'Hình dán không được mua');
        });
    ...
    public function prepareApiUserCallerCallData($userUId, $type)
    {
        ...
        if (!__isEmpty($userOnlineStatus) and $userOnlineStatus == 3) {
            return $this->engineReaction(2, ['show_message' => true], 'Người dùng này không hoạt động');
        }
         // fetch User details
        ...
        //set Call type
        $callType = 1;
        $callTypeTitle = 'Cuộc gọi âm thanh';
        //set Call type
        if ($type == 'audio') {
            $callTypeTitle = 'Cuộc gọi âm thanh';
            $callType   = 1;
        } else if ($type == 'video') {
            $callTypeTitle = 'Cuộc gọi video';
            $callType   = 2;
        }
    ...
    public function prepareUserCallerCallData($userUId, $type)
    {
		...
		if (getStoreSettings('allow_pusher') and getStoreSettings('allow_agora')) {
			...
			//error response
			return $this->engineReaction(2, ['show_message' => true], 'Đã xảy ra sự cố, vui lòng liên hệ với quản trị viên');
		}
    ...
    public function prepareCallerRejectCall($receiverUserUid)
    {
        ...
        // Check if user exists
        if (__isEmpty($user)) {
            return $this->engineReaction(18, null, 'Người dùng không tồn tại');
		}
		$loggedInUser = Auth::user();
		//loggedIn user name
		$loggedInUserName = $loggedInUser->first_name.' '.$loggedInUser->last_name;
    ...
    public function prepareReceiverRejectCall($callerUserUid)
    {
		...
		PushBroadcast::notifyViaPusher('event.call.reject.notification' ,[
			...
			'userUid' 				=> $user->_uid,
			'subject' 				=> 'Người nhận từ chối cuộc gọi',
            'receiverName'          => $loggedInUserName,
			'message' 				=> 'Ngắt kết nối cuộc gọi',
			'messageType' 			=> __tr('success')
		]);
		return $this->engineReaction(1, null);
	}
    ...
    public function prepareReceiverCallAccept($receiverUserUid)
    {
		...
		PushBroadcast::notifyViaPusher('event.call.accept.notification' ,[
			...
			'userUid' 				=> $user->_uid,
			'subject' 				=> 'Người nhận chấp nhận cuộc gọi',
            'receiverName'          => $loggedInUserName,
