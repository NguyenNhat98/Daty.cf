    ...
    public function accountActivation(Request $request, $userUid)
    {
        ...
        if ($processReaction['reaction_code'] === 1) {
            return redirect()->route('user.login')
                        ->with([
                            'success' => 'true',
                            'message' => 'Tại khoản của bạn đã được kích hoạt thành công. Đăng nhập bằng email và mật khẩu của bạn',
                        ]);
        }
        ...
        return redirect()->route('user.login')
                        ->with([
                            'error' => 'true',
                            'message' => 'Liên kết kích hoạt tài khoản không hợp lệ',
                        ]);
	}
    ...
    public function newEmailActivation(Request $request, $userUid, $newEmail)
    {
        ...
        if ($processReaction['reaction_code'] === 1) {
           return redirect()->route('user.new_email.read.success')->with([
                'success' => true,
                'message' => 'Email mới của bạn đã được kích hoạt thành công',
             ]);
        }
        // if activation process failed then
        return redirect()->route('user.new_email.read.success')->with([
                'success' => false,
                'message' => 'Email không được cập nhật',
             ]);
	}
