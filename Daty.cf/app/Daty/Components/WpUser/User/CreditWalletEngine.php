...
use App\Daty\Components\WpUser\User\Repositories\{CreditWalletRepository, ManageUserRepository};
use App\Daty\Components\WpAdmin\Configuration\Repositories\ConfigurationRepository;
use App\Daty\Components\WpAdmin\CreditPackage\Repositories\CreditPackageRepository;
use App\Daty\Components\WpUser\User\PaypalEngine;
use App\Daty\Components\WpUser\User\StripeEngine;
...
class CreditWalletEngine extends BaseEngine
{
    ...
    public function processPaypalTransaction($inputData, $packageUid)
    {
		...
		if ($paypalPaymentDetail['reaction_code'] == 1) {
			...
			return $this->engineReaction(2, [
				'errorMessage' => 'Đã xảy ra sự cố, vui lòng liên hệ với quản trị viên hệ thống'
			], 'Payment Failed');
		}
	}
        ...
        if ($paypalPaymentData['reaction_code'] == 1) {
            ...
            if ($paymentStatus == 'VERIFIED' and $state == 'approved') {
                ...
                if ($this->storePaymentData($paypalResponse, $packageUid, 'apiPaypalPayment')) {
                    return $this->engineReaction(1, null, 'Hoàn tất thanh toán');
                }
            } else {
                //payment failed response
                return $this->engineReaction(2, null, 'Thanh toán không thành công');
            }
        } else {
            //error response
            return $this->engineReaction(2, [
    ...
    public function processPayment($inputData)
    {
		...
		if (__isEmpty($packageCollection)) {
			//success function
			return $this->engineReaction(2, null, 'Gói không tồn tại');
		}
		...
		if ($paymentMethod == 'stripe') {
			$stripeRequestData = [
				...
				'amount'		=> $packageCollection['price']/23000,
				'currency'		=> getStoreSettings('currency'),
			];
		...
		//failure response
		return $this->engineReaction(2, null, 'Đã xảy ra sự cố, vui lòng liên hệ với quản trị viên hệ thống');
	}
    ...
    public function storeStripePaymentData($inputData, $packageUid)
    {
		...
		if (!__isEmpty($inputData)) {
			...
			if ($this->creditWalletRepository->storeTransaction($storeData, $packageCollection)) {
				//success function
				return $this->engineReaction(1, null, 'Giao dịch được lưu trữ');
			}
		}
		//error response
		return $this->engineReaction(2, null, 'Giao dịch không được lưu trữ');
	}
    ...
    public function storePaymentData($inputData, $packageUid, $paymentMethod)
    {
		...
		if (!__isEmpty($inputData)) {
			...
			if ($financialTransactionId = $this->creditWalletRepository->storeTransaction($storeData, $packageCollection)) {
				...
				totalUserCredits();
				//success function
				return $this->engineReaction(1, null, 'Giao dịch không lưu trữ');
			}
		}
		return $this->engineReaction(2, null, 'Giao dịch không được lưu trữ');
	}
