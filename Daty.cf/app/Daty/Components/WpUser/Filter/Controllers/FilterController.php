...
namespace App\Daty\Components\WpUser\Filter\Controllers;
    ...
    public function getFindMatches(CommonUnsecuredPostRequest $request)
    {
        ...
        if ($request->ajax()) {
            return $this->responseAction(
                ...
                $this->replaceView('WpUser.filter.find-matches', $processReaction['data'])
            );
        } else {
            return $this->loadPublicView('WpUser.filter.filter', $processReaction['data']);
        }
    }
}
