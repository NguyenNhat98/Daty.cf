...
namespace App\Daty\Components\WpAdmin\Configuration;
...
class ConfigurationEngine extends BaseEngine
{
    ...
    public function prepareConfigurations($pageType)
    {
        ...
        if (__isEmpty($defaultSettings)) {
            return $this->engineReaction(18, null, 'Loại trang không hợp lệ');
        }
	...
	public function processConfigurationsStore($pageType, $inputData)
    {
        ...
        if (!__isEmpty($inputData)) {
            ...
            if ($isDataAddedOrUpdated) {
                return $this->engineReaction(1, ['show_message' => true], 'Cấu hình đã được cập nhật thành công');
            }
