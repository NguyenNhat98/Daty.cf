...
namespace App\Daty\Components\WpAdmin\User;

use App\Daty\Base\BaseEngine;
use App\Daty\Components\WpAdmin\User\Repositories\{ManageUserRepository, CreditWalletRepository};
...
'gender',
    ...
    public function processGenerateFakeUser($options)
    {
    	$transactionResponse = $this->manageUserRepository->processTransaction(function() use($options) {
            ...
            if ($addedUsersIds) {
				...
				foreach ($addedUsersIds as $key => $addedUserID) {
				    ...
				    $profiles[] = [
				        ...
				        'gender' 			=> $gender,
				        'profile_picture' 	=> strtr("https://picsum.photos/id/__imageID__/360/360.jpg", ['__imageID__' => array_rand($photoIds) ]),
				        'cover_picture' 	=> strtr("https://picsum.photos/id/__imageID__/820/360.jpg", ['__imageID__' => array_rand($photoIds) ]),
				        'dob' 				=> Carbon::now()->subYears($age)->format('d-m-Y'),
            ...
            if ($usersAdded and $authoritiesAdded and $profilesAdded) {
            	return $this->manageUserRepository->transactionResponse(1, ['show_message' => true], 'Đã thêm người dùng giả mạo thành công');
            }

            // // Send failed server error message
            return $this->manageUserRepository->transactionResponse(2, ['show_message' => true], 'Người dùng giả mạo không được thêm vào');
    ...
    public function processUserPhotoDelete($userUid, $type, $profileOrPhotoUid)
    {
    	$transactionResponse = $this->manageUserRepository->processTransaction(function() use($userUid, $type, $profileOrPhotoUid) {
	        ...
	        // check if user details exists
	        if (__isEmpty($userDetails)) {
	            return $this->manageUserRepository->transactionResponse(18, null, 'Người dùng không tồn tại');
	        }
	        ...
	        if ($type == 'photo') {
	        	...
	        	if ($this->manageUserRepository->deleteUserPhoto($userPhoto)) {
	        		...
	        		return $this->manageUserRepository->transactionResponse(1,['show_message' => true], 'Ảnh đã được xóa thành công');
	        	}
	        ...
	        return $this->manageUserRepository->transactionResponse(2, ['show_message' => true], 'Đã có lỗi trên máy chủ');
        });
