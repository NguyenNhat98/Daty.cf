...
@section('head-title', 'Cập nhật hồ sơ')
@section('keywordName', 'Cập nhật hồ sơ')
@section('keyword', 'Cập nhật hồ sơ')
@section('description', 'Cập nhật hồ sơ')
@section('keywordDescription', 'Cập nhật hồ sơ')
...
<style>
	.lw-login-register-page .lw-page-bg {
		background-image: url('<?= __yesset("imgs/auto/*.jpg", false, [
            'random' => true
        ]) ?>');
	}
</style>
...
<div class="container">
	...
	<div class="card mt-5">
		<div class="card-body text-center">
			...
			<a class="float-right" href="#" data-toggle="modal" data-target="#logoutModal">
				<i class="fas fa-sign-out-alt fa-sm fa-fw text-gray-400"></i><strong>Đăng xuất</strong>
			</a>
			<hr>
			<h5>
                Vui lòng hoàn thành hồ sơ của bạn
			</h5>
		</div>
	</div>
	<div id="smartwizard" class="lw-update-wizard">
		<ul class="justify-content-center d-none d-md-flex">
			<li>
				<a class="h5" href="#step-1"><i class="fas fa-images"></i><br>Ảnh đại diện</a>
			</li>
			<li>
				<a class="h5" href="#step-2"><i class="fas fa-map-marker-alt"></i><br>Chọn vị trí
				</a>
			</li>
			<li>
				<a class="h5" href="#step-3"><i class="fas fa-check"></i><br>Hoàn thành
				</a>
			</li>
		</ul>
		...
		<div>
							...
							<form class="lw-ajax-form lw-form" lwSubmitOnChange method="post" data-show-message="true" action="<?= route('user.write.update_profile_wizard') ?>" data-callback="checkProfileStatus">
								...
								<div class="form-row">
									...
									<div class="col-lg-6">
										...
										<input type="text" name="birthday" value="<?= __ifIsset($profileInfo['birthday'], $profileInfo['birthday']) ?>" placeholder="YYYY-MM-DD" class="form-control" required dateISO="true">
									</div>
									<!-- /Birthday -->
							...
							<div class="row" id="lwProfileAndCoverEditBlock">
								<div class="col-lg-4 mt-4">
									<input type="file"
										   ...
										   data-label-idle="Kéo và thả ảnh của bạn</span>"
				...
				<div class="card-body">
					...
					<div id="lwUserEditableLocation">
						<div class="form-group">
							<label for="address_address">Vị trí</label>
			...
			<div id="step-3" class="">
				...
				<div class="text-center ">
					<a href class="btn btn-primary p-3 text-white lw-ajax-link-action" data-method="post" data-action="<?= route('user.profile.finish_wizard') ?>" data-callback="finishWizardCallback"> <strong>Hoàn thành</strong></a>
				</div>
			</div>
		</div>
	</div>
		...
		function initialize() {
			...
			for (let i = 0; i < autocompletes.length; i++) {
				...
				google.maps.event.addListener(autocomplete, 'place_changed', function () {
					...
					if (!place.geometry) {
						window.alert("Không có thông tin chi tiết nào để nhập: '" + place.name + "'");
						input.value = "";
						return;
					}
