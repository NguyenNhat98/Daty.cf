...
@section('head-title', 'Tìm kiếm')
@section('keywordName', 'Tìm kiếm')
@section('keyword', 'Tìm kiếm')
@section('description', 'Tìm kiếm')
@section('keywordDescription', 'Find Matches')
...
<div class="card lw-find-form-container mb-4 ">
    ...
    <form class="form-inline mr-auto" data-show-processing="true" action="<?= route('user.read.find_matches') ?>">
            ...
            <div class="lw-looking-for-container lw-basic-filter-field">
                <label for="lookingFor"><?= __tr('Giới tính') ?></label>
                <select name="looking_for" class="form-control" id="lookingFor">
                    <option value="all"><?= __tr('Tất cả') ?></option>
            ...
            <div class="lw-age-between-container lw-basic-filter-field">
                <label for="min_Age">Độ tuổi</label>
...
{{--
                <label class="justify-content-start" for="distance"><?= __tr('Khoảng cách __distanceUnit__', ['__distanceUnit__' =>( getStoreSettings('distance_measurement') == '6371') ? 'KM' : 'Miles']) ?></label>
--}}
                <label class="justify-content-start" for="distance">Khoảng cách KM ?</label>

                <input type="text" class="form-control" name="distance"
                value="<?= (request()->distance != null) ? request()->distance : getUserSettings('distance') ?>" placeholder="Khoảng cách">
            </div>
            <!-- /Distance from my location -->
            <div class="lw-basic-filter-footer-container lw-basic-filter-field">
                <button type="submit" class="btn btn-primary">Tìm kiếm</button>
                <a href class="btn btn-secondary" style="<?= !__isEmpty(request()->is_advance_filter) ? 'display: none;' : '' ?>" id="lwShowAdvanceFilterLink"><i class="fas fa-filter"></i> Hiển thị bộ lọc nâng cao</a>
                <a href class="btn btn-secondary" style="<?= __isEmpty(request()->is_advance_filter) ? 'display: none;' : '' ?>" id="lwHideAdvanceFilterLink"><i class="fas fa-filter"></i> Ẩn bộ lọc nâng cao</a>
            </div>
        </form>
</div>
</div>
...
<!-- Found matches container -->
        ...
        <div class="lw-advance-filter-tabs">
            ...
            <form class="" action="<?= route('user.read.find_matches') ?>">
                                    ...
                                    @if($item['input_type'] == 'select')
                                        @if($itemKey == 'height')
                                            ...
                                            <div class="lw-specification-select-box">
                                                ...
                                                <option value="" selected>Chiều cao tối thiểu</option>
                                                ...
                                                <option value="" selected>Chiều cao tối đa</option>
                                                @foreach($item['options'] as $optionKey => $option)
                ...
                <div class="lw-search-button-container">
                    <button type="submit" class="btn btn-primary btn-block-on-mobile">Áp dụng</button>
                </div>
            </form>
        </div>
        <!-- /Tabs for advance filter -->
	...
	<?= __trn('__filterCount__ Trùng khớp','__filterCount__ Trùng khớp', $totalCount, ["__filterCount__" => $totalCount]) ?></div>
    <!-- /Advance Filter Options -->
    <div class="row row-cols-sm-1 row-cols-md-2 row-cols-lg-4" id="lwUserFilterContainer">
        @if(!__isEmpty($filterData))
            @include('WpUser.filter.find-matches')
        @endif
    </div>
    @if($hasMorePages)
