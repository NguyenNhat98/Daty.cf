	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<script type="text/_template" id="lwIncomingCallTemplate"
				...
				<br><br>
				<!-- accept call button -->
				<button class="btn btn-secondary" id="lwAcceptCall" data-response-data="<%- JSON.stringify(__tData.responseData) %>" data-show-if="receiverAcceptCallBtn"><i class="fas fa-phone-volume"></i> Chấp nhận</button>
				<!-- /accept call button -->

				<!-- disconnect call button -->
				<button class="btn btn-danger" id="lwReceiverDisConnectCallBtn" data-response-data="<%- JSON.stringify(__tData.responseData) %>" data-show-if="receiverDisconnectCallBtn"><i class="fas fa-phone-slash"></i> Từ chối</button>
				<!-- /disconnect call button -->

				<!-- close call button -->
				<button class="btn btn-light" style="display: none" data-show-if="receiverCloseCallBtn" id="lwReceiverCloseCallBtn" type="button" data-dismiss="modal" aria-label="Close">Đóng</button>
				<!-- /close call button -->
...
<div id="lwCallErrorContainer" style="display: none;">
    <h3>Lỗi !</h3>
    <strong>Có thể trình duyệt của bạn không hỗ trợ Cuộc gọi âm thanh / Video. Chúng tôi khuyên bạn nên sử dụng trình duyệt khác </strong>
</div>
<!-- User Soft delete Container -->
...
<script>
	...
	if (userLoggedIn && enablePusher && enableAgora) {
		...
		var __callStatusStrings = {
			...
			'disconnect' : 'Cuộc gọi đã ngắt kết nối ...',
				...
				__DataRequest.post(audioVideoCallUrl, null, function (responseData) {
					...
					if (responseData.reaction == 1) {
						...
						__Utils.modalTemplatize('#lwAudioCallTemplate', function(e, data) {
							...
							//get call type
							var callTypeTitle = 'Cuộc gọi âm thanh';
		...
		subscribeNotification('event.call.notification', pusherAppKey, userUid, function(responseData) {
			...
			if (isCompatible) {
				...
				__Utils.modalTemplatize('#lwIncomingCallTemplate', function(e, data) {
					//get call type
					var callTypeTitle = "Cuộc gọi âm thanh đến";
					if (responseData.callType == 1) {
						callTypeTitle = "Cuộc gọi âm thanh đến";
					} else if (responseData.callType == 2) {
						callTypeTitle = "Cuộc gọi điện video đến";
					}
					return {
						'callType'  : 	callTypeTitle,
