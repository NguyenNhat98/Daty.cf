    ...
    <div class="col-md-2 text-right lw-messenger-user-menu">
        <div class="dropdown">
			...
			<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">
                ...
                <a class="dropdown-item" id="lwAudioCallDisableBtn"><i class="fas fa-phone-alt"></i> Cuộc gọi âm thanh</a>
				<!-- audio call button -->
				<a class="dropdown-item" href type="button" id="lwAudioCallBtn" data-user-uid="<?= $userData['user_uid'] ?>" data-call-type="audio" data-confirm="#lwCallErrorContainer"><i class="fas fa-phone-alt"></i> Cuộc gọi âm thanh</a>
				<!-- /audio call button -->
				@endif

				@if(getFeatureSettings('video_call_via_messenger'))
                <a class="dropdown-item" id="lwVideoCallDisableBtn" readonly><i class="fas fa-video"></i> Cuộc gọi video</a>
				<!-- video call button -->
				<a class="dropdown-item" href type="button" id="lwVideoCallBtn" data-user-uid="<?= $userData['user_uid'] ?>" data-call-type="video" data-confirm="#lwCallErrorContainer"><i class="fas fa-video"></i> Cuộc gọi video</a>
				<!-- /video call button -->
				@endif
				@endif

				<!-- delete all chat button -->
                <a class="dropdown-item lw-disable-link" id="lwDeleteAllChatDisableButton" readonly><i class="fas fa-trash"></i> Xóa tất cả trò chuyện</a>
				<a class="dropdown-item lw-ajax-link-action" id="lwDeleteAllChatActiveButton" href="<?= route('user.write.delete_all_messages', ['userId' => $userData['user_id']]) ?>" data-method="post" data-callback="userChatResponse" data-post-data='<?= json_encode(['to_user_id' => $userData['user_id']]) ?>' type="button"><i class="fas fa-trash"></i>Xóa tất cả trò chuyện</a>
				<!-- /delete all chat button -->
			</div>
        </div>
    </div>
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body py-4 bg-white text-center">
				<h5>Người dùng cần chấp nhận yêu cầu trò chuyện</h5>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary lw-not-accepted-dialog-close-btn">Đóng</button>
			</div>
		</div>
	</div>
</div>
