@section('page-title', 'Trang chủ')
@section('head-title', 'Trang chủ')
@section('keywordName', 'Trang chủ')
@section('keyword', 'Trang chủ')
@section('description', 'Trang chủ')
@section('keywordDescription', 'Trang chủ')
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
	<h4 class="h5 mb-0 text-gray-200">
		<span class="text-primary"><i class="fas fa-fire"></i></span> Gặp gỡ gợi ý
	</h4>
</div>
...
@if(getFeatureSettings('user_encounter'))
	@if(!__isEmpty($randomUserData))
		...
		<div class="lw-random-user-block">
			@if($randomUserData['isPremiumUser'])
				<span class="lw-premium-badge" title="Người dùng cao cấp"></span>
		@endif
		<!-- user name -->
			<div class="lw-user-text">
				...
				@if($randomUserData['userOnlineStatus'])
					@if($randomUserData['userOnlineStatus'] == 1)
						<span class="lw-dot lw-dot-success float-right" title="Hoạt động"></span>
					@elseif($randomUserData['userOnlineStatus'] == 2)
						<span class="lw-dot lw-dot-warning float-right" title="Không dùng"></span>
					@elseif($randomUserData['userOnlineStatus'] == 3)
						<span class="lw-dot lw-dot-danger float-right" title="Không hoạt động"></span>
				@endif
			@endif
			<!-- /show user online, idle or offline status -->
			</div>
	...
	@else
		...
		<div class="alert alert-info">
			Giới hạn cuộc gặp gỡ hàng ngày của bạn có thể vượt quá hoặc không có người dùng nào để hiển thị.		</div>
		<!-- / info message -->
	@endif
@else
	<!-- info message -->
...
--}}
		Đây là một tính năng cao cấp, để xem cuộc gặp gỡ, trước tiên bạn cần mua gói cao cấp.
	</div>
	<!-- / info message -->
@endif
<!-- /user encounter main container -->
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
	<h4 class="h5 mb-0 text-gray-200">
		<span class="text-primary"><i class="fas fa-users"></i></span> Ngẫu nhiên
	</h4>
</div>
