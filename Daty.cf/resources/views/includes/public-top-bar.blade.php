    ...
    <ul class="navbar-nav ml-0">
        ...
        <li class="nav-item dropdown no-arrow">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <i class="fas fa-search fa-sm"></i>&nbsp;<span class="d-md-block d-none">Tìm kiếm</span>
			</a>
            ...
            <div class="dropdown-menu p-3 shadow animated--grow-in lw-basic-filter-container" aria-labelledby="searchDropdown">
                <form class="form-inline mr-auto" data-show-processing="true" action="<?= route('user.read.find_matches') ?>">
                    ...
                    <div class="lw-looking-for-container lw-basic-filter-field">
                        <label for="looking_for">Giới tính</label>
...
--}}
                        ...
                        <label for="distance">Khoảng cách từ vị trí của tôi (KM)</label>
	...
	@if(!isPremiumUser())
	<a href="<?= route('user.premium_plan.read.view') ?>" class="btn btn-primary btn-sm" title="NGƯỜI DÙNG CAO CẤP">CAO CẤP</a>
	<!-- /buy premium plans page link -->
	@endif
	<!-- Topbar Navbar -->
            ...
            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="alertsDropdown">
				...
				<script type="text/_template" id="lwNotificationListTemplate">
					<% if(!_.isEmpty(__tData.notificationList)) { %>
						...
						<a class="dropdown-item text-center small text-gray-500" href="<?= route('user.notification.read.view') ?>" id="lwShowAllNotifyLink" data-show-if="showAllNotifyLink">Hiển thị tất cả thông báo</a>
						<!-- /show all notification link -->
					<% } else { %>
						<!-- info message -->
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="boosterModalLabel">Tăng cường hồ sơ</h5>
			...
			<div class="alert alert-info" id="lwBoosterCreditsNotAvailable" style="display: none;">
                Số dư tài khoản của bạn quá thấp, vui lòng
				<a href="<?= route('user.credit_wallet.read.view') ?>">Nạp tài khoản</a>
			</div>
			<!-- / insufficient balance error message -->

			<div class="text-center">

                Điều này sẽ khiến bạn phải mất phí
		  ...
		  <div class="modal-footer">
			<button type="button" class="btn btn-light btn-sm" data-dismiss="modal">Thoát</button>
			<a class="btn btn-success btn-sm lw-ajax-link-action" data-callback="onProfileBoosted" href="<?= route('user.write.boost_profile') ?>" data-method="post"><i class="fas fa-bolt fa-fw"></i> Tăng cường</a>
		  </div>
		</div>
	</div>
</div>
