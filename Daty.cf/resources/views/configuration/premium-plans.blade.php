	...
	<div class="row">
		...
		<div class="col-sm-6">
			...
			<div class="mb-3" id="lwPlanPriceInputField_<?= $key ?>">
                <label for="lwPrice_<?=$key ?>">Giá</label>
				<input type="number" class="form-control form-control-user" value="<?= $plan['price'] ?>" name="plan_duration[<?=$key ?>][price]" placeholder="Giá" id="lwPrice_<?=$key ?>">
			</div>
			<!-- / Plan Price -->
		</div>
		@endforeach
